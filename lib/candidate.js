...
class Candidate extends Base {
  ...
  validate() {
    ...
    if (this.contest.get('ranked') && (!ranks || !ranks.length)) {
      throw new Error('Contest marked as ranked choice, but candidate does not have ranks. ' + this.contest.get('contestID') + ' | ' + this.get('entryID'));
    }
  }

  // Update names
  updateNames() {
    this.set('middle', this.get('middle') ?
      this.get('middle').replace(/^([a-z])$/i, '$1.') : undefined);
    this.set('suffix', this.get('suffix') ?
      this.get('suffix').replace(/(jr|sr)\.?/i, '$1.') : undefined);
    this.set('title', this.get('title') ?
      this.get('title').replace(/(mrs|miss|ms|mr)\.?/i, '') : undefined);
  }
  ...
  displayName() {
    ...
    return _.filter([
      this.get('title'), this.get('prefix'),
      this.get('first'), this.get('middle'),
      this.get('nick') ? '"' + this.get('nick') + '"' : null,
      this.get('last'), this.get('suffix')
    ]).join(' ');
  }
  ...
  sortName() {
    ...
    return _.filter([this.get('last'), !this.get('first'), !this.get('middle')]).join(' ');
  }

  // Sort by party value, prioritize major parties.
  sortParty() {
    return this.get('party').match(/^(dfl|d|r)$/i) ? 'AAAAAA' + this.get('party') : this.get('party');
  }

  // Make candidate ID
  id(contest, candidate) {
    contest = contest || this.contest;
    candidate = candidate || this._props;
    return utility.makeID([contest.type, contest.county, contest.district,
      contest.office, candidate.candidate]);
  }
