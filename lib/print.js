...
function toPrint(contests, section) {
  ...
  _.each(contests, c => {
    ...
    _.each(section.headings || defaultHeadings, (h, hi) => {
      if (h) {
        let currentH = _.isArray(h) ? _.filter(_.pick(c, h)).join(' ') : c[h];
        let previousH = previous
          ? _.isArray(h) ? _.filter(_.pick(previous, h)).join(' ') : previous[h]
          : undefined;

        // If heading changed or no previous or high heading changed
        if (
          (higherHeadingChanged || !previous || currentH !== previousH) &&
          currentH
        ) {
          ...
          if (
            !(
              currentH &&
              ~currentH.toLowerCase().indexOf('question') &&
              c.question
            )
          ) {
    ...
    _.each(c.candidates, (candidate) => {
      ...
      if (c.ranked) {
        lines.push(
          tags.ranked +
            (candidate.winner && !c.close
              ? '<saxo:ch value="226 136 154"/>'
              : ' ') +
            '\t' +
            display(candidate) +
