...
class Election extends Base {
  ...
  exportPrint() {
    ...
    _.each(sections, section => {
      ...
      // Get set of contests
      let subset = this.filterContests(section.where, contests);
      debug('Print section: ', section.title, subset.length);

      // Sort
      subset = _.sortBy(subset, _.filter(section.headings));
      if (!subset.length) {
        return;
      }
      ...
      _.each(subset, c => {
        ...
        _.each(section.headings || defaultHeadings, (h, hi) => {
          if (h) {
            ...
            if (
              ...
              lines.push(tags['heading' + (hi + 2)] + c[h]);
        ...
        // Question text
        if (c.questionText) {
          lines.push(tags.question + c.questionText);
        }
          ...
          if (c.ranked) {
            ...
            lines.push(
              tags.candidate +
                ...
                '\t' +
                (candidate.votes
                  ? utility.formatNumber(candidate.votes, 0)
                  : 0) +
                '\t' +
  ...
  sortContests(contests) {
    ...
    contests = _.orderBy(
      ...
      [
        ...
        c => {
          return c.get('subArea');
        },
        c => {
          return c.get('seatName');
        }
      ],
      ['asc', 'asc', 'asc', 'asc', 'asc']
    );
