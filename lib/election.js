...
class Election extends Base {
  ...
  exportPrint() {
    ...
    _.each(sections, section => {
      ...
      // Get set of contests
      let subset = _.filter(contests, c => {
        // Find contests that match all the where clauses
        let failed = null;
        _.find(section.where, (v, k) => {
          failed = !(_.isString(v) && _.isString(c[k])
            ? c[k].match(new RegExp(v, 'i'))
            : _.isEqual(c[k], v));
      ...
      _.each(subset, c => {
        ...
        _.each(section.headings, (h, hi) => {
          if (h) {
            // If changed or no previous
            if ((!previous || previous[h] !== c[h]) && c[h]) {
