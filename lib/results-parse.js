...
function parse(row, config = {}) {
  ...
  let d = {
    ...
    countyID: common.padLeft(common.raw(parts[1]), 2),
    precinctID: common.padLeft(common.raw(parts[2]), 4),
    officeID: common.padLeft(common.raw(parts[3]), 4),
    officeRaw: common.raw(parts[4]),
    // District can be many different things.
    districtID: common.raw(parts[5]),
    candidateID: common.padLeft(common.raw(parts[6]), 4),
  ...
  // Make IDs
  d.contestID = common.makeContestID(d);
  d.entryID = common.makeEntryID(d, { candidateID: d.candidateID });
