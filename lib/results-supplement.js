...
// Main function
async function supplementAggregatedResults(aggregated, election = {}, options = {}) {
  ...
  // Connect to base
  const base = airtable.base(process.env.AIRTABLE_BASE_ID || election.supplement.id);

  // Possible cache
  let { contests, results } = await source(options, base);
...
async function source(options, base) {
  // Use cache
  if (options.useCache) {
