...
class Publish extends Base {
  constructor(election, options) {
    ...
    if (this.s3Location.protocol !== 's3:') {
      throw new Error(
        's3Location option provided does not use the s3:// protocol.'
      );
    }
  ...
  publish(version = false) {
    return new Promise((resolve, reject) => {
      ...
      // a timestamp
      let prefix =
        this.s3Location.pathname.replace(/^\/+/g, '') +
        '/' +
        this.election.id();
      ...
      let options = {
        ...
        // Note that --version will be clobbered
        // by not --version.
        deleteRemoved: true,
        s3Params: {
          ...
          ACL: 'public-read',
          CacheControl:
            'public, max-age=' + this.cacheSeconds + ', must-revalidate',
          Expires: moment()
            .add(this.cacheSeconds, 'seconds')
            .toDate()
