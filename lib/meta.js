...
class Meta extends Base {
  ...
  parseSoSQuestions(row, type) {
    ...
    parsed.questionTitle = titlecase(utility.raw(row[5])
  ...
  parseSoSDistricts(row, type) {
    ...
    if (type === 'county') {
      ...
      parsed.name = titlecase(utility.raw(row[1]).toLowerCase())
      ...
      // FIPS code
      parsed.local = utility.padLeft(utility.raw(row[2]), 5);
      parsed.name = titlecase(utility.raw(row[3]).toLowerCase());
      ...
      parsed.name = titlecase(utility.raw(row[2]).toLowerCase());
