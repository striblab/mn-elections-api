...
class Contest extends Base {
  ...
  matchMeta(meta) {
    let districtMatch = utility.makeID([this.election.get('id'), this.get('type'), this.get('district')]);
    ...
    _.each(meta, (m) => {
      ...
      // Districts. There can be multiple districts per contest.
        ...
        this.get('type') === m.get('type') &&
        districtMatch === m.get('contestMatch')) {
  ...
  id(contest) {
    ...
    return utility.makeID([this.election.get('id'), contest.type,
      contest.district, contest.contest]);
  }
  ...
  parseSoS(input, properties = {}) {
    ...
    // meaningful
    if (~['mn-president', 'amendment', 'state', 'judicial'].indexOf(properties.type)) {
      d.district = 'MN';
    }
