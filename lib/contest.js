...
class Contest extends Base {
  ...
  candidatesWinner(candidates) {
    ...
    let losers = _.reject(candidates, c => {
      return c.get('winner') || c.get('writeIn');
    });
    if (losers && losers.length) {
      let leastWinningVotes = _.min(
        _.map(
          _.filter(candidates, c => {
            return c.get('winner');
          }),
          c => {
            return c.get('votes');
      ...
      if (leastWinningVotes && totalVotes) {
        ...
        _.each(losers, c => {
          ...
          if (
            totalVotes <= 400 &&
            Math.abs((c.get('votes') - leastWinningVotes) / totalVotes) <
              10 / 400
          ) {
            this.set('close', true);
          }
          else if (
            Math.abs((c.get('votes') - leastWinningVotes) / totalVotes) < 0.005
          ) {
            this.set('close', true);
          }
        });
  ...
  parseSoS(input, properties = {}) {
    ...
    if (properties.type === 'school' && d.nameRaw.match(/ssd\s#1\)/i)) {
      d.district = '9991';
    }
