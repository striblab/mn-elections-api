...
class Base {
  constructor(options = {}) {
    ...
    this.exportPath = _.isString(options.exportPath) ? options.exportPath : path.join(__dirname, '..', 'mn-elections-output');
  ...
  // Parse party code
  parseSoSParty(input) {
    return _.isString(input) ? input.replace(/\s+/, '').toLowerCase() : undefined;
  }
